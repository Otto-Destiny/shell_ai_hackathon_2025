...
@st.cache_data
def get_all_blends_data(db_path=str(APP_DB_PATH)) -> pd.DataFrame:
...
with st.sidebar:
    st.markdown("---")
    st.markdown("### ðŸ¦… Developed by eagle-team")
    st.markdown("""
    - Destiny Otto
    - Williams Alabi
    - Godswill Otto
...
st.markdown("""
    <div class="header">
        <h1 style='text-align: center; margin-bottom: 0.5rem;'>ðŸ¦… Eagle Blend Optimizer</h1>
...
def add_blends(df, db_path=str(APP_DB_PATH), n_components=5):
    df = df.copy()
...
def add_components(df, db_path=str(APP_DB_PATH)):
    df = df.copy()
...
def get_blends_overview(db_path: str = str(APP_DB_PATH), last_n: int = 5) -> Dict[str, Any]:
    """
    Returns:
      {
...
def get_activity_logs(db_path=str(APP_DB_PATH), timeframe="today", activity_type=None):
    """
...
def get_model(db_path=str(APP_DB_PATH)):
    """
...
with tabs[0]:
    ...
    with k1:
        st.markdown(f"""
            <div class="metric-card" style="padding:10px;">
                ...
                <div class="metric-delta">RÂ² (latest)</div>
            </div>
        """, unsafe_allow_html=True)
    with k2:
        st.markdown(f"""
    ...
    # <label for="help-toggle" class="help-button">ðŸ’¬ Help</label>
    ...
    #     </div>
    #     <div class="help-body">
    #         <p><b>Performance Cards:</b> These show key metrics at a glance. "Model Accuracy" is the latest RÂ² score. "Predictions" and "Optimizations" cover this week's activity. If a card shows "â€”", the underlying data may be missing.</p>
...
# --- FIX: Removed extra blank lines inside the <ul> tag to ensure all items render ---
    st.markdown("""
        ...
        <input id="help-toggle" type="checkbox" />
        <label for="help-toggle" class="help-button">ðŸ’¬ App Guide</label>
        ...
        <div class="help-panel" aria-hidden="true">
            ...
            <div class="help-body">
                ...
                <ul>
                    <li><b>ðŸ“Š Dashboard:</b> You are here! This is your main overview, showing key metrics like model accuracy, recent app activity, and the highest cost savings achieved. The list on the right gives you a live look at the most recently created blends.</li>
                    <li><b>ðŸŽ›ï¸ Blend Designer:</b> This is your creative sandbox. Manually define the fractions and properties of up to five components to instantly predict the final properties of a new blend. You can also switch to <b>Batch Mode</b> to upload a CSV and predict many blends at once.</li>
                    <li><b>âš™ï¸ Optimization Engine:</b> Go beyond simple prediction. Here, you set the <b>target properties</b> you want to achieve. The AI engine will then run an optimization to find the ideal component fractions that best meet your goals and constraints, such as minimizing cost.</li>
                    <li><b>ðŸ“¤ Blend Comparison:</b> This is your analysis workbench. Select up to three previously saved blends from your database to perform a detailed side-by-side comparison. The charts will help you visualize differences in their cost, composition, and performance profiles.</li>
                    <li><b>ðŸ“š Fuel Registry:</b> The heart of your data. This tab is where you manage the database of all raw <b>Components</b> and saved <b>Blends</b>. You can view, add, and delete records here.</li>
                    <li><b>ðŸ§  Model Insights:</b> Look under the hood of the AI. This tab shows detailed performance metrics for the prediction model, helping you understand its accuracy and where its predictions are most reliable.</li>
                </ul>
...
# ---------- Floating "How to Use" (bigger button + inline content) + compact CSS ----------
    ...
    with right_col:
        ...
        st.markdown('<div style="font-size: 1.15rem; font-weight:800; color:#654321; margin-bottom:12px;">ðŸ—’ï¸ Recent Blends</div>', unsafe_allow_html=True)
        ...
        if df_recent is None or df_recent.empty:
            ...
            for _, row in df_recent.iterrows():
                ...
                props_html = " Â· ".join(props) if props else "No properties available."
...
@st.cache_data
def get_components_from_db(db_path=str(APP_DB_PATH)) -> pd.DataFrame:
...
def log_activity(activity_type: str, details: str = "", db_path=str(APP_DB_PATH)):
...
with tabs[1]:
    ...
    def handle_prediction():
        ...
        if abs(sum(fractions) - 1.0) > 0.01:
            st.warning("âš ï¸ Total of component fractions must sum to 1.0.")
            st.session_state.prediction_made = False
            return
...
#     st.success(f"âœ… Prediction complete in {duration:.2f} seconds! Scroll down to see the results.")
...
def handle_save_prediction():
    ...
    with col_header[0]:
        st.subheader("ðŸŽ›ï¸ Blend Designer")
    with col_header[1]:
    ...
    if batch_blend:
        ...
        try:
            with open(BATCH_TEMPLATE_PATH, "rb") as f:
                st.download_button(
                    label="ðŸ“¥ Download Batch Template (CSV)",
                    data=f,
                    file_name="batch_template.csv",
                    mime="text/csv"
                )
        except FileNotFoundError:
            st.warning("Batch template file not found. Please create 'data/templates/batch_template.csv'.")
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("ðŸ§ª Run Batch Prediction", use_container_width=True, type="primary"):
        ...
        # st.button("ðŸ§ª Predict Blended Properties", on_click=handle_prediction, use_container_width=True, type="primary")
        # --- FIX: Changed button call to prevent page jumping ---
        # --- In the "Manual Blend Designer UI" section ---
        if st.button("ðŸ§ª Predict Blended Properties", use_container_width=False, type="primary"):
        ...
        if st.session_state.get('prediction_made', False):
            ...
            st.subheader("ðŸ“Š Visualizations & Actions")
            ...
            if st.button("ðŸ’¾ Save Prediction to Database", use_container_width=False):
    ...
    st.markdown("""
        ...
        <label for="help-toggle-designer" class="help-button-designer">ðŸ’¬ Help</label>
...
with tabs[2]:
    st.subheader("âš™ï¸ Optimization Engine")
    ...
    with main_cols[1]: # Right side for last 2 components and controls
        ...
        with st.container(border=True):
            ...
            with run_button_col:
                # New Code:
                if st.button("ðŸš€ Run Optimization", use_container_width=False, type="primary", disabled=st.session_state.optimization_running):
    ...
    if st.session_state.optimization_results:
        ...
        with save_col:
            ...
            if st.button("ðŸ’¾ Save to Database", use_container_width=False):
                # Prepare DataFrame in the format expected by `add_blends`
                save_data = {}
            ...
            st.download_button("ðŸ“¥ Download All Solutions (CSV)", data="dummy_csv_data", file_name="optimization_results.csv", use_container_width=False)
    ...
    st.markdown("""
        ...
        <label for="help-toggle-optimizer" class="help-button">ðŸ’¬ Help</label>
...
@st.cache_data
def get_blend_property_ranges(db_path=str(APP_DB_PATH)) -> dict:
...
with tabs[3]:
    st.subheader("ðŸ“Š Blend Scenario Comparison")
        ...
        if selected_blends:
            ...
            with plot_cols[1]:
                ...
                if radar_data_exists:
                    ...
                    st.warning("Radar Chart cannot be displayed. The selected blend(s) have no property data in the database.", icon="ðŸ“Š")
            # --- Plot 3 & 4 ---
    ...
    st.markdown("""
        ...
        <label for="help-toggle-comparison" class="help-button">ðŸ’¬ Help</label>
...
def load_data(table_name: str, db_path=str(APP_DB_PATH)) -> pd.DataFrame:
...
def delete_records(table_name: str, ids_to_delete: list, id_column: str, db_path=str(APP_DB_PATH)):
...
with tabs[4]:
    st.subheader("ðŸ“š Fuel Registry")
        ...
        with st.container(border=True):
            ...
            with st.expander("Add a Single Component Manually"):
                with st.form("new_component_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("ðŸ’¾ Save Component", use_container_width=True):
            ...
            st.download_button(
                label="ðŸ“¥ Download Component Template",
                data=get_template(COMPONENTS_TEMPLATE_PATH),
    ...
    with col2:
        with st.container(border=True):
            st.markdown("#### ðŸ§¬ Add Blends")
            st.info("Upload blend compositions via CSV. Manual entry is not supported for blends.", icon="â„¹ï¸")
            ...
            st.download_button(
                label="ðŸ“¥ Download Blend Template",
                data=get_template(BLENDS_TEMPLATE_PATH),
    ...
    st.markdown("#### ðŸ” View & Manage Registry Data")
    ...
    if df_display.empty:
        ...
        if not selected_rows.empty:
            ...
            if st.button(f"âŒ Delete Selected {table_to_show} ({len(selected_rows)})", use_container_width=False, type="primary"):
    ...
    st.markdown("""
        ...
        <label for="help-toggle-registry" class="help-button">ðŸ’¬ Help</label>
...
with tabs[5]:
    ...
    #         <ul>
    #             <li><b>Overall RÂ² Score:</b> Think of this as the model's accuracy grade. A score of 92.4% means the model's predictions are highly accurate.</li>
    ...
    #         </ul>
    #         <p><b>RÂ² Score by Blend Property Chart:</b> This chart shows how well the model predicts each specific property.</p>
...
# --- FIX: Complete working version of the help button ---
    st.markdown("""
        ...
        <label for="help-toggle-insights" class="help-button">ðŸ’¬ Help</label>
        <div class="help-panel help-panel-insights">
            ...
            <div class="help-body">
                <p><b>KPI Cards:</b> These cards give a quick summary of the model's health. <b>RÂ² Score</b> is its accuracy grade, while <b>MSE</b> and <b>MAPE</b> measure the average size of its errors.</p>
                <p><b>RÂ² Score by Blend Property Chart:</b> This chart shows how well the model predicts each specific property. A longer bar means the model is very good at predicting that property.</p>
            </div>
        </div>
    """, unsafe_allow_html=True)

    # --- Main Title ---
    st.markdown('<h2 style="color:#4a2f1f; font-size:1.75rem;">ðŸ§  Model Insights</h2>', unsafe_allow_html=True)
    ...
    with k2:
        st.markdown(f"""
            <div class="metric-card">
                <div class="metric-label">Overall RÂ² Score</div>
    ...
    st.markdown('<h3 style="color:#4a2f1f; font-size:1.5rem;">RÂ² Score by Blend Property</h3>', unsafe_allow_html=True)
    ...
    fig_r2.add_trace(go.Bar(
        ...
        marker=dict(
            ...
            colorscale='YlOrBr',
            colorbar=dict(title="RÂ² Score", tickfont=dict(color="#4a2f1f")),
        ),
    ...
    fig_r2.update_layout(
        xaxis_title="RÂ² Score (Higher is Better)",
